...
void MonodzukuriKinovaDemo_wayPointDCompliant::start(
  ...
  ctl.compShoulderTask->positionTask->stiffness(50);
  // ctl.compShoulderTask->positionTask->damping(30.0);
  ctl.compShoulderTask->positionTask->weight(10000);
  ctl.compShoulderTask->orientationTask->stiffness(50);
  // ctl.compShoulderTask->orientationTask->damping(30.0);
  ctl.compShoulderTask->orientationTask->weight(10000);
  ...
  ctl.compPostureTask->stiffness(0);
  ctl.compPostureTask->weight(0);
  ctl.compPostureTask->makeCompliant(true);
  ctl.compEETask->reset();
  ctl.compEETask->positionTask->stiffness(50);
...
bool MonodzukuriKinovaDemo_wayPointDCompliant::run(mc_control::fsm::Controller &ctl_) {
  ...
  if(ctl.compEETask->eval().norm() < 0.1 && ctl.compShoulderTask->eval().norm() < 0.1)
