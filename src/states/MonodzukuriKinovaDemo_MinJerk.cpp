...
void MonodzukuriKinovaDemo_MinJerk::configure(
...
void MonodzukuriKinovaDemo_MinJerk::start(mc_control::fsm::Controller &ctl_) {
  ...
  target_circle(0) =
      ctl.target_pose.first - robot_radius +
      new_target_pos.first * (2.0 * robot_radius) / win_size.first;
  target_circle(1) =
      ctl.target_pose.second - robot_radius +
      new_target_pos.second * (2.0 * robot_radius) / win_size.second;
...
bool MonodzukuriKinovaDemo_MinJerk::run(mc_control::fsm::Controller &ctl_) {
  ...
  if (!transitionStarted_) {
    ...
    ctl.game.setRobotPosition(pose(1), pose(2), ctl.target_pose.first,
                              ctl.target_pose.second);
...
void MonodzukuriKinovaDemo_MinJerk::save_last_target(
    mc_control::fsm::Controller &ctl_) {
  auto &ctl = static_cast<MonodzukuriKinovaDemo &>(ctl_);
  auto robot_radius = ctl.game.getRobotRadius();
  auto win_size = ctl.game.getWinSize();
  ...
  mj_task->W(W);
  projection_vector =
      target_circle - controlled_frame->position().translation().tail<2>();

  FittsData data = ctl.game.getLastData();
  ctl.game.clearNewTargetBool();
}

EXPORT_SINGLE_STATE("MonodzukuriKinovaDemo_MinJerk",
                    MonodzukuriKinovaDemo_MinJerk)
