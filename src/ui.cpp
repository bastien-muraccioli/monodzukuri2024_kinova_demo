...
// Constructor
FittsGame::FittsGame()
: win_size(1920, 1080), win_fitts_size(570, 570), pointer_radius(3.0f), wait_time(1.0f), target_idx(-1)
{
}
...
void FittsGame::run()
{
  ...
  mc_rtc::log::info("pointer_pos: ({}, {})", pointer_pos.first, pointer_pos.second);
  ...
  window = glfwCreateWindow(win_size.first, win_size.second, "Futur Monodzukuri 2024",  nullptr, nullptr);
  if(!window)
  {
...
void FittsGame::manageBackground(void)
{
  switch(control_mode)
  {
    case MC_MODE:
      ...
      {
        renderTexture(mc_defaultID, win_size.first/2, win_size.second/2, 1080.0f);
      }
      break;
    case DC_MODE:
      if(jrl_torque)
      ...
      {
        renderTexture(dc_defaultID, win_size.first/2, win_size.second/2, 1080.0f);
      }
      break;
    case FC_MODE:
      if(jrl_torque)
      ...
      {
        renderTexture(fc_defaultID, win_size.first/2, win_size.second/2, 1080.0f);
      }
      break;
    case MJ_MODE:
      if(jrl_torque)
      ...
      {
        renderTexture(mj_defaultID, win_size.first/2, win_size.second/2, 1080.0f);
      }
      break;
    case NS_MODE:
      if(jrl_torque)
      ...
      {
        renderTexture(ns_defaultID, win_size.first/2, win_size.second/2, 1080.0f);
      }
      break;
    default:
      break;
...
void FittsGame::setControlMode(int mode)
{
  control_mode = mode;
}

void FittsGame::setJRLTorque(bool torque)
{
  jrl_torque = torque;
}
...
position FittsGame::newRandomPose(float radius)
{
  ...
  do
  {
    ...
    valid = min_fitts_x + radius < new_x and new_x < max_fitts_x - radius and min_fitts_y + radius < new_y and new_y < max_fitts_y - radius;
...
std::string FittsGame::getPath(const std::string & filename) {
    ...
    std::filesystem::path source_dir = source_file.parent_path();  // Get the source directory
...
void FittsGame::renderTexture(GLuint texture, float x, float y, float scale)
{
  ...
  // Get the texture's width and height (assuming you have these values stored when loading the texture)
  int width, height;
  ...
  // // Define the texture coordinate (0, 1) and the top-left corner of the quad.
  // glTexCoord2f(0.0f, 1.0f);
  // glVertex2f(-0.5f, 0.5f);

  // // Define the texture coordinate (1, 1) and the top-right corner of the quad.
  // glTexCoord2f(1.0f, 1.0f);
  // glVertex2f(0.5f, 0.5f);

  // // Define the texture coordinate (1, 0) and the bottom-right corner of the quad.
  // glTexCoord2f(1.0f, 0.0f);
  // glVertex2f(0.5f, -0.5f);

  // // Define the texture coordinate (0, 0) and the bottom-left corner of the quad.
  // glTexCoord2f(0.0f, 0.0f);
  // glVertex2f(-0.5f, -0.5f);
  glEnd();
